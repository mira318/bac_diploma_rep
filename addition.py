...
'''проблемы: использует один GPU -- потомучто на ноутбуке он один 
нет параметра --aug-repeats=3 -- датасет должен быть распределён по нодам -- снова проблема с одним GPU
неправильные количества эпох -- чтобы можно было хотя бы проверить, что всё остальное работает. Количества сохранены в изветсный файл и не один раз.'''
